...
export function AssistantMessage({
  ...
  useEffect(() => {
    ...
    // 1. TTS 启用
    // 2. 是最后一条消息
    // 3. 消息刚刚完成（isLoading 从 true 变为 false）
    // 4. 有内容和角色
    // 5. 当前会话未播放过
    ...
    if (
      ...
      contentString &&
      characterId &&
      !hasPlayedInSession.current
    ) {
      hasPlayedInSession.current = true;
      // 延迟一点时间播放，让 UI 先渲染
      setTimeout(() => {
        speak({ text: contentString, character: characterId });
      }, 500);
    }
  }, [ttsEnabled, isLoading, contentString, characterId, isLastMessage]);
